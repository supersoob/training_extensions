...
from ote.backends.torch.mmcv.job import MMJob
...
class MMClsJob(MMJob):
    def configure(self, cfg: Config, model_cfg=None, data_cfg=None, **kwargs):
        ...
        if model_ckpt is not None:
            cfg.load_from = MMJob.get_model_ckpt(model_ckpt)
    ...
    def configure_task(cfg, training, model_meta=None, **kwargs):
        ...
        train_data_cfg = MMJob.get_train_data_cfg(cfg)
        if isinstance(train_data_cfg, list):
            train_data_cfg = train_data_cfg[0]

        model_classes = MMClsJob.get_model_classes(cfg)
        data_classes = MMClsJob.get_data_classes(cfg)
        if model_classes:
    ...
    def get_data_classes(cfg):
        data_classes = []
        train_cfg = MMJob.get_train_data_cfg(cfg)
    ...
    def get_model_classes(cfg):
        ...
        if len(classes) == 0:
            ...
            if ckpt_path:
                classes = MMJob.read_label_schema(ckpt_path)
        if len(classes) == 0:
