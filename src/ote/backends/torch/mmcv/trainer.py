...
class MMTrainer(MMTask):
    ...
    def run(self, model, data, recipe, **kwargs):
        ...
        if hasattr(cfg, 'seed'):
            MMClsTrainer._set_random_seed(cfg.seed, deterministic=cfg.get('deterministic', False))
        else:
            cfg.seed = None
        ...
        if distributed:
            ...
            mp.spawn(MMTrainer.train_worker, nprocs=len(cfg.gpu_ids),
                     args=(model, datasets, val_datasets, cfg, distributed, True, timestamp, meta))
        else:
            MMTrainer.train_worker(None, model, datasets, val_datasets,
                                   cfg,
                                   distributed,
                                   True,
                                   timestamp,
    ...
    def train_worker(gpu, model, dataset, val_dataset, cfg, distributed, validate, timestamp, meta):
        ...
        train_data_cfg = MMTrainer.get_train_data_cfg(cfg)
        ...
        if MMTrainer.is_gpu_available():
            if distributed:
                ...
                model = MMDistributedDataParallel(
                    model,
                    device_ids=[MMTrainer.get_current_device()],
        ...
        if cfg.get('checkpoint_config', False):
            runner.register_hook(MMTrainer.register_checkpoint_hook(cfg.checkpoint_config))
