...
class Recipe:
    def __init__(self, recipe_yaml):
        ...
        for config in task_config.tasks:
            ...
            if spec in self.tasks:
                ...
                continue
            self.tasks[spec] = Recipe._build_task(cfg)
    ...
    def train(self, **kwargs):
        ...
        self.tasks[spec].configure(self.recipe,
        ...
        results = self._run_task(spec, self.model, datasets, **kwargs)
    ...
    def eval(self, metric, **kwargs):
        ...
        return self._run_task("evaluator", metric, infer_results, **kwargs)

    def infer(self, **kwargs):
        self.model = self._get_model()
        dataset = kwargs.get("dataset", self.adapters["data"].get_test_dataset())
        return self._run_task("inferrer", model, dataset, **kwargs)

    def export_model(self, **kwargs):
        model = self._get_model()
        return self._run_task("exporter", model, **kwargs)
