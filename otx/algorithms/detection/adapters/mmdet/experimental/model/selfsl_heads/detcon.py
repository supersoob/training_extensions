...
class MaskPooling(nn.Module):
    ...
    def pool_masks(self, masks: torch.Tensor):
        ...
        if self.overlap_region['type'] == 'ignore' and self.overlap_region['value'] > 0:
            overlapped_region = (masks.sum(dim=1) >= self.overlap_region['value']).to(torch.float)
    ...
    def sample_masks(self, masks: List[torch.Tensor], overlapped_region: Optional[List[torch.Tensor]] = None):
        """Samples which binary masks to use in the loss.
        Args:
            masks: [(b, num_classes, d), (b, num_classes, d)]
            overlapped_region: (Optional)
        Returns:
...
def _init_weights(module: nn.Module, init_linear: str = 'normal', std: float = 0.01, bias: float = 0.):
...
class DetConB(SAMDetectorMixin, L2SPDetectorMixin, SingleStageDetector):
    ...
    Args:
        overlap_region: overwrite, ignore (with ratio), distance
    """
    def __init__(
        self,
        backbone,
        ...
        in_index=None,
        overlap_region={'type': 'overwrite', 'value': -1},
        loss_weights=None,
        pretrained=None,
        **kwargs
    ):
        ...
        self.mask_pool = MaskPooling(num_classes, num_samples, downsample, overlap_region)
        ...
        self.projector_online.init_weights(init_linear='kaiming')
...
class DetConBSupCon(DetConB):
    def _convert_bboxes_to_masks(self, masks: torch.Tensor, bboxes, labels):
        """
        Convert annotations: bboxes to masks
        """
        if self.overlap_region['type'] == 'distance':
        ...
        for bbox, label in zip(bboxes, labels):
            ...
            elif self.overlap_region['type'] == 'ignore' and self.overlap_region['value'] > 0:
